(() => {
    ...
    function render() {
        ...
        const filtered = entries.filter((entry) => { const haystack = `${entry.name} ${entry.location} ${entry.role} ${entry.gifts}`.toLowerCase(), isMet = met.has(entry.name); return (!query || haystack.includes(query)) && (location === 'all' || entry.location === location) && (role === 'all' || entry.role === role) && (status === 'all' || (status === 'met' ? isMet : !isMet)); });
