...
function detail(html){const text=plain(html),value=Number(text.match(/SELL VALUE\s+(\d+)/i)?.[1]),energy=Number(text.match(/ENERGY BOOST\s+\+?(\d+)/i)?.[1]);return {sellValue:Number.isFinite(value)?value:null,energy:Number.isFinite(energy)?energy:null,availability:/Unavailable\s+[—-]\s+Event ended/i.test(text)?'Event ended':'Available',sourceSlug:null};}
const failures=[];
