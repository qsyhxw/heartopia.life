...
function buildPerson(slug, person) {
  ...
  const detailCards = person.details.map(([heading, content]) => `<section class="rounded-xl bg-white p-5 border border-[#E7BEAA] shadow-sm"><h2 class="font-display text-xl font-bold mb-2">${esc(heading)}</h2><p class="text-cozy-wood leading-relaxed">${esc(content)}</p></section>`).join('');
