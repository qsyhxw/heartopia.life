...
const expectedProgressVersion = createHash('sha256').update(progressSource.replace(/\r\n/g, '\n')).digest('hex').slice(0, 12);
const expectedAssistantVersion = createHash('sha256').update(progressAssistantSource.replace(/\r\n/g, '\n')).digest('hex').slice(0, 12);
...
check(progressPage.includes(`/assets/js/my-progress-assistant.js?v=${expectedAssistantVersion}`), 'My Progress: assistant cache version is stale');
