...
function card(event, compact = false) {
  ...
  if (profile.hubOnly) {
    const source = profile.officialUrl ? `<a class="mt-5 inline-flex w-fit items-center gap-1 font-bold text-cozy-bark underline hover:text-cozy-coral" href="${esc(profile.officialUrl)}" target="_blank" rel="noopener noreferrer nofollow">Official announcement <span aria-hidden="true">↗</span><span class="sr-only"> (external link)</span></a>` : '';
    return `<article class="card flex h-full flex-col overflow-hidden">${content}${source}</div></article>`;
  }
  return `<a class="card event-card group flex h-full flex-col overflow-hidden" href="/events/${route(event)}/" aria-label="View ${esc(event.name)} event guide">${content}<span class="mt-5 inline-flex w-fit items-center gap-2 rounded-lg bg-cozy-bark px-3.5 py-2 text-sm font-bold text-white transition-colors group-hover:bg-cozy-coral">View guide <span aria-hidden="true">&rarr;</span></span></div></a>`;
}
...
export function renderHeartopiaEvents(events, updatedAt) {
  for (const event of events) {
    if (profileFor(event).hubOnly) continue;
